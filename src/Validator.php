...
use Repack\Validation\Support\Arr;
use Repack\Validation\Support\MessageBag;
...
class Validator
{
     ...
     * @var array
     */
    protected $implicitAttributes = array();
     ...
     * @var array
     */
    protected $dependentRules = array(
     ...
     * @return void
     */
    public function __construct($translator, array $data, array $rules,
        array $messages = array(), array $customAttributes = array()) {
    ...
    public function sometimes($attribute, $rules, $callback)
    {
        ...
        if (call_user_func($callback, $payload)) {
            foreach ((array) $attribute as $key) {
                if (Str::contains($key, '*')) {
                    $this->explodeRules(array($key => $rules));
                } else {
     ...
     * @return $this
     */
    public function mergeRules($attribute, $rules = array())
    {
    ...
    protected function validateAttribute($attribute, $rule)
    {
        ...
        if (
            $value instanceof SplFileInfo && !$value->isValid() &&
            $this->hasRule($attribute, array_merge($this->fileRules, $this->implicitRules))
        ) {
            return $this->addFailure($attribute, 'uploaded', array());
        }
    ...
    {
        if (!$this->hasRule($attribute, array('Nullable'))) {
            return true;
        }
    ...
    {
        if ($this->hasRule($attribute, array('Bail'))) {
    ...
    protected function validateMax($attribute, $value, $parameters)
    {
        ...
        if ($value instanceof SplFileInfo && !$value->isValid()) {
            return false;
        }
    ...
    {
        return Str::contains($table, '.') ? explode('.', $table, 2) : array(null, $table);
    }
    ...
    protected function validateDimensions($attribute, $value, $parameters)
    {
        ...
        if (isset($parameters['ratio'])) {
            list($numerator, $denominator) = array_replace(
                array(1, 1), array_filter(sscanf($parameters['ratio'], '%f/%d'))
            );
    ...
    protected function getCustomMessageFromTranslator($customKey)
    {
        ...
        foreach ($customMessages as $key => $message) {
            if ($shortKey === $key || (Str::contains($key, array('*')) && Str::is($key, $shortKey))) {
                return $message;
            }
        }
    ...
    protected function getAttributeType($attribute)
    {
        ...
        if ($this->hasRule($attribute, $this->numericRules)) {
            ...
            return 'array';
        } elseif ($this->getValue($attribute) instanceof SplFileInfo) {
            return 'file';
        }
    ...
    protected function doReplacements($message, $attribute, $rule, $parameters)
    {
        ...
        $message = str_replace(
            ...
            array($value, mb_strtoupper($value), Str::ucfirst($value)),
            $message
        );
    ...
    protected function getAttribute($attribute)
    {
        ...
        $expectedAttributes = $attribute != $primaryAttribute ? array($attribute, $primaryAttribute) : array($attribute);
    ...
    protected function replaceRequiredUnless($message, $attribute, $rule, $parameters)
    {
        ...
        return str_replace(array(':other', ':values'), array($other, implode(', ', $parameters)), $message);
    }
    ...
    {
        return array(Str::studly(trim(Arr::get($rules, 0))), array_slice($rules, 1));
    }
    ...
    protected function callClassBasedExtension($callback, $parameters)
    {
        if (Str::contains($callback, '@')) {
            ...
            list($class, $method) = array($callback, 'validate');
        }
    ...
    protected function callClassBasedReplacer($callback, $message, $attribute, $rule, $parameters)
    {
        if (Str::contains($callback, '@')) {
            ...
            list($class, $method) = array($callback, 'replace');
        }
